...
public class Oveja extends Animal implements IHerviboro, IMuerto, IReproducible{
    public Oveja (String name, int pX, int pY){
        super.setName(name);
    ...
    public void muerto(Animal animal) {
        //System.out.println("La oveja: " + super.getName() + " fue deborado :(");
    ...
    public void comerPlantas(int posX, int posY) {
        if(Mapa.matrizNueva[posX][posY] instanceof Planta){
            ...
            System.out.println("La oveja: " + this.getName() + " está comiendo: " + laPlanta.getName());
    ...
    public void creandoOvejasBebes() {
        Animal ovejaCreada = nombrePosicionOvejas();
        Mapa.matrizCambios[ovejaCreada.getPosicionX()][ovejaCreada.getPosicionY()] = ovejaCreada.getName();
        Mapa.matrizNueva[ovejaCreada.getPosicionX()][ovejaCreada.getPosicionY()] = ovejaCreada;

        System.out.println("Ha nacido una nueva oveja: " + ovejaCreada.getName());
    }
    public Animal nombrePosicionOvejas(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal ovejita = new Oveja("\uD83D\uDC11", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=ovejita;
        }else{
            ovejita = nombrePosicionOvejas();
        }
        return ovejita;
    }
}
