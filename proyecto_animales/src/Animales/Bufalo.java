...
public class Bufalo extends Animal implements IHerviboro, IMuerto, IReproducible{
    public Bufalo (String name, int pX, int pY){
        super.setName(name);
    ...
    public void muerto(Animal animal) {
        //System.out.println("El bufalo: " + super.getName() + " fue deborado :(");
    ...
    public void comerPlantas(int posX, int posY) {
        if(Mapa.matrizNueva[posX][posY] instanceof Planta){
            ...
            System.out.println("El bufalo: " + this.getName() + " está comiendo: " + laPlanta.getName());
    ...
    public void creandoBufalosBebes() {
        Animal bufaloCreado = nombrePosicionBufalos();
        Mapa.matrizCambios[bufaloCreado.getPosicionX()][bufaloCreado.getPosicionY()] = bufaloCreado.getName();
        Mapa.matrizNueva[bufaloCreado.getPosicionX()][bufaloCreado.getPosicionY()] = bufaloCreado;

        System.out.println("Ha nacido una nuevo bufalo: " + bufaloCreado.getName());
    }
    public Animal nombrePosicionBufalos(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal bufalito = new Bufalo("\uD83D\uDC03", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=bufalito;
        }else{
            bufalito = nombrePosicionBufalos();
        }
        return bufalito;
    }
}
