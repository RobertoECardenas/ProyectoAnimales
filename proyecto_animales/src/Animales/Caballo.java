...
public class Caballo extends Animal implements IHerviboro, IMuerto, IReproducible{
    public Caballo (String name, int pX, int pY){
        super.setName(name);
    ...
    public void muerto(Animal animal) {
        //System.out.println("El caballo: " + super.getName() + " fue deborado :(");
    ...
    public void comerPlantas(int posX, int posY) {
        if(Mapa.matrizNueva[posX][posY] instanceof Planta){
            ...
            System.out.println("El caballo: " + this.getName() + " está comiendo: " + laPlanta.getName());
    ...
    public void reproduce() {
        ...
        int posibilidad = randomNum.nextInt(11);

        switch (posibilidad){
            case 0:
                creandoCaballosBebes();
                break;
            case 1:
                break;
            case 2:
    ...
    public void creandoCaballosBebes() {
        Animal caballoCreado = nombrePosicionCaballos();
        Mapa.matrizCambios[caballoCreado.getPosicionX()][caballoCreado.getPosicionY()] = caballoCreado.getName();
        Mapa.matrizNueva[caballoCreado.getPosicionX()][caballoCreado.getPosicionY()] = caballoCreado;

        System.out.println("Ha nacido un nuevo caballo: " + caballoCreado.getName());
    }
    public Animal nombrePosicionCaballos(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal caballito = new Caballo("\uD83D\uDC0E", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=caballito;
        }else{
            caballito = nombrePosicionCaballos();
        }
        return caballito;
    }
}
