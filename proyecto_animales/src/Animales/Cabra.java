...
public class Cabra extends Animal implements IHerviboro, IMuerto, IReproducible{
    public Cabra (String name, int pX, int pY){
        super.setName(name);
    ...
    public void muerto(Animal animal) {
        //System.out.println("La cabra: " + super.getName() + " fue deborada :(");
    ...
    public void comerPlantas(int posX, int posY) {
        if(Mapa.matrizNueva[posX][posY] instanceof Planta){
            ...
            System.out.println("La cabra: " + this.getName() + " está comiendo: " + laPlanta.getName());
            laPlanta.comida(laPlanta);
        }
    }
    public void creandoCabrasBebes() {
        Animal cabritaCreada = nombrePosicionCabras();
        Mapa.matrizCambios[cabritaCreada.getPosicionX()][cabritaCreada.getPosicionY()] = cabritaCreada.getName();
        Mapa.matrizNueva[cabritaCreada.getPosicionX()][cabritaCreada.getPosicionY()] = cabritaCreada;

        System.out.println("Ha nacido una nueva cabra: " + cabritaCreada.getName());
    }
    public Animal nombrePosicionCabras(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal cabrita = new Cabra("\uD83D\uDC10", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=cabrita;
        }else{
            cabrita = nombrePosicionCabras();
        }
        return cabrita;
    }
    @Override
    public void reproduce() {
        ...
        int posibilidad = randomNum.nextInt(8);
