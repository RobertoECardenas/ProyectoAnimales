...
public class Zorro extends Animal implements ICarnivoro,IReproducible,IMuerto{
    public Zorro (String name, int pX, int pY){
        super.setName(name);
    ...
    public void cazar(int posX, int posY){
        if (Mapa.matrizNueva[posX][posY] != null){
            if(Mapa.matrizNueva[posX][posY] instanceof Conejo){
                ...
                System.out.println("El zorro: " + this.getName() + " está comiendo: " + elConejo.getName());
                elConejo.muerto(elConejo);
            }
            if(Mapa.matrizNueva[posX][posY] instanceof Raton){
                Raton elRaton = (Raton) Mapa.matrizNueva[posX][posY];
                System.out.println("El zorro: " + this.getName() + " está comiendo: " + elRaton.getName());
                elRaton.muerto(elRaton);
            }
            if(Mapa.matrizNueva[posX][posY] instanceof Pato){
                Pato elPato = (Pato) Mapa.matrizNueva[posX][posY];
                System.out.println("El zorro: " + this.getName() + " está comiendo: " + elPato.getName());
                elPato.muerto(elPato);
            }
            if(Mapa.matrizNueva[posX][posY] instanceof Oruga){
                Oruga elOruga = (Oruga) Mapa.matrizNueva[posX][posY];
                System.out.println("El zorro: " + this.getName() + " está comiendo: " + elOruga.getName());
                elOruga.muerto(elOruga);
            }
        }
    }
    ...
    public void creandoZorrosBebes() {
        Animal zorrosCreados = nombrePosicionZorros();
        Mapa.matrizCambios[zorrosCreados.getPosicionX()][zorrosCreados.getPosicionY()] = zorrosCreados.getName();
        Mapa.matrizNueva[zorrosCreados.getPosicionX()][zorrosCreados.getPosicionY()] = zorrosCreados;

        System.out.println("Ha nacido un nuevo zorro: " + zorrosCreados.getName());
    }
    public Animal nombrePosicionZorros(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal zorrito = new Zorro("\uD83E\uDD8A", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=zorrito;
        }else{
            zorrito = nombrePosicionZorros();
        }
        return zorrito;
    }
    @Override
    public void muerto(Animal animal) {
        //System.out.println("El zorro: " + super.getName() + " fue deborado :(");
