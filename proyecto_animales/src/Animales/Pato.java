...
public class Pato extends Animal implements IHerviboro, IMuerto, IReproducible, ICarnivoro{
    public Pato (String name, int pX, int pY){
        super.setName(name);
    ...
    public void muerto(Animal animal) {
        //System.out.println("El pato: " + super.getName() + " fue deborado :(");
    ...
    public void comerPlantas(int posX, int posY) {
        if(Mapa.matrizNueva[posX][posY] instanceof Planta){
            ...
            System.out.println("El pato: " + this.getName() + " está comiendo: " + laPlanta.getName());
    ...
    public void creandoPatosBebes() {
        Animal patoCreado = nombrePosicionPatos();
        Mapa.matrizCambios[patoCreado.getPosicionX()][patoCreado.getPosicionY()] = patoCreado.getName();
        Mapa.matrizNueva[patoCreado.getPosicionX()][patoCreado.getPosicionY()] = patoCreado;

        System.out.println("Ha nacido un nuevo pato: " + patoCreado.getName());
    }
    public Animal nombrePosicionPatos(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal patito = new Pato("\uD83E\uDD86", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=patito;
        }else{
            patito = nombrePosicionPatos();
        }
        return patito;
    }
    ...
    public void cazar(int posX, int posY) {
        if (Mapa.matrizNueva[posX][posY] != null){
            if(Mapa.matrizNueva[posX][posY] instanceof Oruga){
                Oruga laOruga = (Oruga) Mapa.matrizNueva[posX][posY];
                System.out.println("El Pato: " + this.getName() + " está comiendo: " + laOruga.getName());
                laOruga.muerto(laOruga);
            }
        }
    }
}
