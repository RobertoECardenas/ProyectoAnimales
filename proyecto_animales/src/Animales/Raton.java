...
public class Raton extends Animal implements IHerviboro, IMuerto, IReproducible{
    public Raton (String name, int pX, int pY){
        super.setName(name);
    ...
    public void muerto(Animal animal) {
        //System.out.println("El raton: " + super.getName() + " fue deborado :(");
    ...
    public void comerPlantas(int posX, int posY) {
        if(Mapa.matrizNueva[posX][posY] instanceof Planta){
            ...
            System.out.println("El raton: " + this.getName() + " está comiendo: " + laPlanta.getName());
    ...
    public void creandoRatonesBebes() {
        Animal ratonCreado = nombrePosicionRatones();
        Mapa.matrizCambios[ratonCreado.getPosicionX()][ratonCreado.getPosicionY()] = ratonCreado.getName();
        Mapa.matrizNueva[ratonCreado.getPosicionX()][ratonCreado.getPosicionY()] = ratonCreado;

        System.out.println("Ha nacido un nuevo raton: " + ratonCreado.getName());
    }
    public Animal nombrePosicionRatones(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal ratoncito = new Raton("\uD83D\uDC01", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=ratoncito;
        }else{
            ratoncito = nombrePosicionRatones();
        }
        return ratoncito;
    }
}
