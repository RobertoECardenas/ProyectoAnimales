...
public class Oruga extends Animal implements IHerviboro, IMuerto, IReproducible{
    public Oruga (String name, int pX, int pY){
        super.setName(name);
    ...
    public void muerto(Animal animal) {
        //System.out.println("La oruga: " + super.getName() + " fue deborado :(");
    ...
    public void comerPlantas(int posX, int posY) {
        if(Mapa.matrizNueva[posX][posY] instanceof Planta){
            ...
            System.out.println("La oruga: " + this.getName() + " está comiendo: " + laPlanta.getName());
    ...
    public void creandoOrugasBebes() {
        Animal orugaCreada = nombrePosicionOrugas();
        Mapa.matrizCambios[orugaCreada.getPosicionX()][orugaCreada.getPosicionY()] = orugaCreada.getName();
        Mapa.matrizNueva[orugaCreada.getPosicionX()][orugaCreada.getPosicionY()] = orugaCreada;

        System.out.println("Ha nacido una nueva oruga: " + orugaCreada.getName());
    }
    public Animal nombrePosicionOrugas(){
        Random randomX = new Random();
        Random randomY = new Random();
        int posicionX = randomX.nextInt(CreadorObjetos.a);
        int posicionY = randomY.nextInt(CreadorObjetos.b);
        Animal oruguita = new Oruga("\uD83D\uDC1B", posicionX, posicionY);
        if(Mapa.matrizCambios[posicionX][posicionY] == null){

            Mapa.matrizCambios[posicionX][posicionY]=oruguita;
        }else{
            oruguita = nombrePosicionOrugas();
        }
        return oruguita;
    }
}
